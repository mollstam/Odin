...
def clear_commands_output():
    with open(json_output_path, 'w') as f:
        f.write(json.dumps({ "entries": [] }))

def write_commands_output(command, output):
    print(output, end='\n')
    all_output = []
...
def execute_command(command_interpreter, command):
    ...
    print("(lldb) {}".format(command))
    command_interpreter.HandleCommand(command, res)

    if res.Succeeded():
        command_output = normalize_whitespace(res.GetOutput() or '')
        write_commands_output(command, command_output)
        ...
        while len(new_breakpoints) > 0:
            ...
            if breakpoint_id in registered_breakpoints:
                ...
                callback_command = ("breakpoint command add -F lldb_batchmode.breakpoint_callback " +
                                    str(breakpoint_id))
...
if __name__ == '__main__':
    if len(sys.argv) != 4:
        print("usage: python lldb_batchmode.py target-path script-path json-output-path")
        sys.exit(1)
